...
import { evidenceIsCurrent, exactEvidence, familyEvidence } from "./state.js";
...
export function deterministicShadow(state, { operation, session, commandHash }) {
  ...
  if (!evidenceIsCurrent(state, evidence)) return { decision: "stale_after_mutation", source: "code", confidence: 1, evidenceGeneration: evidence.generation ?? evidence.workspaceGeneration };
  return { decision: "would_reuse", source: "code", confidence: 1, evidenceGeneration: evidence.generation ?? evidence.workspaceGeneration };
}
...
export async function semanticShadow(state, { operation, session, commandHash }) {
  ...
  if (!related || !evidenceIsCurrent(state, related)) return null;
